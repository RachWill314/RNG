...
onMounted(() => {
    Mqtt.connect();
    //payload.value = { "ledA": 0, "ledB": 0, "number": 0 };
    console.log(payload.number);
    setTimeout(() => {
        // Subscribe to each topic
        Mqtt.subscribe("620155671");
        Mqtt.subscribe("rachelle/topic2+");
    }, 3000);
});
...
console.log(message);
Mqtt.publish("620155671_sub",message); // Publish message to appropriate topic
}
...
<v-icon v-if="payload.ledA == 1" size="50" icon="mdi:mdi-lightbulb-on" color="yellow" ></v-icon>
                    ...
                    <v-card-item>
                        <v-btn class="text-caption" text="Toggle" variant="tonal" color="primary"
                            density="compact" @click="toggle('LED A')"></v-btn>
                    </v-card-item>
                </v-card>
            </v-col>
            <v-col class="col col2" align="center">
                <v-card title="LED B" width="150" density="compact" border flat rounded="md">
                    ...
                    <v-card-item>
                        <v-btn class="text-caption" text="Toggle" variant="tonal" color="primary"
                            density="compact" @click="toggle('LED B')"></v-btn>
                    </v-card-item>
                </v-card>
            </v-col>
        </v-row>
